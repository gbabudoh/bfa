  ...
  return (
    <>
      ...
      <div className="mb-8">
        ...
        <div className={`bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 ${isFiltersOpen ? 'max-h-[500px]' : 'max-h-0 md:max-h-[500px]'}`}>
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Business Type Filter */}
              <div>
                <h3 className="font-medium text-gray-900 mb-3">{t('businessType')}</h3>
              ...
              {/* Badge Type Filter */}
              <div>
                <h3 className="font-medium text-gray-900 mb-3">{t('verificationLevel')}</h3>
                <div className="space-y-2">
                  <div className="flex items-center">
                    <input
                      type="radio"
              ...
              {/* Country Filter */}
              <div>
                <h3 className="font-medium text-gray-900 mb-3">{t('countryLabel')}</h3>
                <select
                ...
                <h3 className="font-medium text-gray-900 mt-4 mb-3">{t('categoryLabel')}</h3>
                <select
      ...
      {(selectedBusinessType !== 'all' || selectedBadge !== 'all' || selectedCountry !== 'all' || selectedCategory !== 'all' || searchQuery) && (
        <div className="mb-6 flex flex-wrap gap-2 items-center">
          <span className="text-sm text-gray-700">{t('activeFilters')}</span>
          ...
          {selectedBusinessType !== 'all' && (
            <div className="bg-yellow-100 text-yellow-800 text-xs px-3 py-1 rounded-full flex items-center">
              {t('typeLabel')} {selectedBusinessType === 'artisan' ? t('artisanMakers') : 
                        selectedBusinessType === 'brand' ? t('brandDesigners') :
                        selectedBusinessType === 'factory' ? t('manufacturerFactories') :
                        selectedBusinessType === 'export' ? t('distributorExport') : selectedBusinessType}
              <button 
                className="ml-1.5 text-yellow-800 hover:text-yellow-900"
          ...
          {selectedBadge !== 'all' && (
            <div className="bg-yellow-100 text-yellow-800 text-xs px-3 py-1 rounded-full flex items-center">
              {t('badgeLabel')} {selectedBadge === 'gold' ? t('goldBadge') : t('blueBadge')}
              <button 
                className="ml-1.5 text-yellow-800 hover:text-yellow-900"
          ...
          {selectedCountry !== 'all' && (
            <div className="bg-yellow-100 text-yellow-800 text-xs px-3 py-1 rounded-full flex items-center">
              {t('countryLabel')} {tc(countries.find(c => c.id === selectedCountry)?.nameKey || selectedCountry)}
              <button 
                className="ml-1.5 text-yellow-800 hover:text-yellow-900"
          ...
          {selectedCategory !== 'all' && (
            <div className="bg-yellow-100 text-yellow-800 text-xs px-3 py-1 rounded-full flex items-center">
              {t('categoryLabel')} {tct(selectedCategory)}
              <button 
                className="ml-1.5 text-yellow-800 hover:text-yellow-900"
              ...
              <div key={vendor.id} className="bg-white rounded-xl shadow-md hover:shadow-lg transition overflow-hidden border border-gray-100">
                    ...
                    <span className={`${businessTypeInfo.color} px-2 py-1 rounded-full text-xs font-medium flex items-center`}>
                      {businessTypeInfo.icon}
                          {vendor.businessType === 'artisan' ? t('artisanBadge') : 
                           vendor.businessType === 'brand' ? t('brandBadge') :
                           vendor.businessType === 'factory' ? t('factoryBadge') :
                           vendor.businessType === 'export' ? t('exportBadge') : t(vendor.businessType || 'artisanBadge')}
                    </span>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center text-xs text-gray-500 mb-2">
                        ...
                        <span className="mr-1">{flag}</span> {vendor.location || t('noLocation')}
                      </div>
                ...
                {vendor.products && vendor.products.length > 0 && (
                  <div className="px-5 pb-3 pt-1 border-t border-gray-50">
                    <h4 className="text-[10px] font-black text-gray-400 mb-2 tracking-widest uppercase">{t('spotlight')}</h4>
